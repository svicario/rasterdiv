Hill <- function(x, window=3, alpha=1, n.processes=1, cluster.type="SOCK", debugging=FALSE) {
  ...
  if (n.processes == 1){
    if(mode == "single") {
      if( BergerParker ) {
        outS <- 1/BergerParkerS(rasterm, w, debugging)
      }
      else if( Shannon ) {
      ...
      for (ALPHA in alpha){
        ...
        if((abs(ALPHA-1)<.Machine$double.eps)) {
          ...
          s <- "Berger-Parker"
          out[[s]] <- 1/BergerParkerS(rasterm, w, debugging)
        }
        else{
