...
Renyi <- function(input, window=3, mode="single", BergerParker=FALSE, alpha=1, base=exp(1), rescale=FALSE, na.tolerance=0.0, simplify=3, nc.cores=1, cluster.type="MPI", debugging=FALSE, integer=FALSE, ...){
  #
  ...
  if (nc.cores == 1){
    if(mode == "single") {
      if( Shannon ) {
        outS <- ShannonS(rasterm, w, base, debugging)
        message(("\nCalculation of Shannon's index is complete!\n"))
      } # End ShannonD
      else if( BergerParker ) {
        outS <- BergerParkerS(rasterm, w, base, debugging)
    ...
    return (outS)
      ...
      for (ALPHA in start:stop){
        if(ALPHA == 1) {
          s <- "ShannonAlpha 1"
          out[[s]] <- ShannonS(rasterm, w, base, debugging)
      ...
      message(("\nCalculation of Rényi's index complete.\n"))
        if (BergerParker){
          s<-"Berger-Parker"
          out[[s]] <- BergerParkerS(rasterm, w, base, debugging)
  ...
  if (nc.cores>1){
    ...
    gc()
      ...
      if(mode == "single") {
      if( Shannon ) {
        outP <- ShannonP(rasterm, w, base, debugging)
      }
      else if (BergerParker){
        outP <- BergerParkerP(rasterm, w, base, debugging)
      }
      else{
      ...
      for (ALPHA in start:stop){
        if(ALPHA == 1) {
          s <- "ShannonAlpha 1"
          out<- ShannonP(rasterm, w, base, debugging)
      ...
      if (BergerParker){
        s <- "Berger-Parker"
        out <- BergerParkerP(rasterm, w, base, debugging)
        outP[[s]] <- do.call(cbind,out)
        message(("\nCalculation of Berger-Parker's index is also complete!\n"))
      }
        return(outP)
...
ShannonS <- function(rasterm, w, base, debugging){
...
BergerParkerS <- function(rasterm, w, base, debugging){
  out<-matrix(rep(NA,dim(rasterm)[1]*dim(rasterm)[2]),nrow=dim(rasterm)[1],ncol=dim(rasterm)[2])
...
ShannonP<-function(rasterm,w,base, debugging){
  #
  ## Reshape values
  #
  values <- as.numeric( as.factor(rasterm) )
...
BergerParkerP<-function(rasterm,w,base, debugging){
  #
  ## Reshape values
  #
  values<-as.numeric(as.factor(rasterm))
...
plot(raster(M1))
r<-Renyi(M1, mode = "sequential", nc.cores = 1, alpha=c(0,4))
r
