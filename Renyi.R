Renyi <- function(input, window=3, alpha=1, base=exp(1), n.process=1, cluster.type="SOCK", debugging=FALSE, ...){
  #
  ## Initial checks
  #
  ...
  #Assign the mode according to the length of alpha
  ...
  if( is(input[[1]],"RasterLayer") ) {
    ...
    #Print user messages
    if (mode == "single"){
      message("Matrix check OK: \nRenyi with alpha parameter value=", alpha," output matrix will be returned.")
    } else if (mode == "iterative"){
